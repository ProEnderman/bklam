  ...
  const flushPatch = useCallback(async (immediate = false) => {
    ...
    const doFlush = async () => {
      ...
      try {
        ...
        // Это критично для корректной работы перетаскивания новых элементов
        ...
        if (tempIds.length === newServerItems.length && tempIds.length > 0) {
          tempIds.forEach((tempId, idx) => {
            if (newServerItems[idx]) {
              tempToServerIdMapRef.current.set(tempId, newServerItems[idx].id)
              console.log(`[HallEditor] Pre-mapped temp ID ${tempId} -> server ID ${newServerItems[idx].id}`)
            }
          })
