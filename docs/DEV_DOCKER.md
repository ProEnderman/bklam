...
- **Вход в приложение:** откройте в браузере фронтенд (если поднят) или логин по API: email `admin@gmail.com`, пароль `admin123` (сначала запрос кода, затем верификация — зависит от настройки отправки писем).
...
Чтобы бот реально получал события, ваш backend должен быть доступен по **публичному HTTPS-URL**. При запуске всего в Docker на своей машине (localhost) Telegram до вас «достучаться» не может. Варианты: поднять туннель (например **ngrok**: `ngrok http 8080`) и прописать в боте URL вида `https://xxx.ngrok.io/api/telegram/webhook`, либо деплой на сервер с белым IP/доменом.
